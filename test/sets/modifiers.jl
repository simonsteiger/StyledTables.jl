@testset "relabel!" begin
    ...
    tbl = StyledTable(df)
    relabel!(tbl, :x => "Variable X", :y => "Variable Y")
    run_reftest(tbl, "references/relabel/relabeled_two_cols")

    tbl = StyledTable(df)
    relabel!(tbl, :x => "Only X")
    run_reftest(tbl, "references/relabel/relabeled_one_col")

    @test_throws ArgumentError relabel!(StyledTable(df), :typo => "Label")

    @testset "mixed-type pairs error" begin
        let df = DataFrame(; a = [1], b = [2])
            # Mixed value types: String and Multiline labels
            @test_throws ArgumentError relabel!(
                StyledTable(df),
...
@testset "relabel! input types" begin
    ...
    let tbl = StyledTable(df)
        relabel!(tbl, Pair{Symbol,Symbol}[:x=>:VarX, :y=>:VarY])
    ...
    let tbl = StyledTable(df)
        relabel!(tbl, Pair{String,Symbol}["x"=>:VarX, "y"=>:VarY])
        @test html_str(tbl) == ref
    end
    ...
    let tbl = StyledTable(df)
        relabel!(tbl, Dict{Symbol,Symbol}(:x => :VarX, :y => :VarY))
        @test html_str(tbl) == ref
    end

    # AbstractDict{AbstractString, AbstractString}
    let tbl = StyledTable(df)
        relabel!(tbl, Dict{String,String}("x" => "VarX", "y" => "VarY"))
        @test html_str(tbl) == ref
    end

    # AbstractDict{AbstractString, Symbol}
    let tbl = StyledTable(df)
        relabel!(tbl, Dict{String,Symbol}("x" => :VarX, "y" => :VarY))
        @test html_str(tbl) == ref
    end

    # AbstractDict{Symbol, AbstractString}
    let tbl = StyledTable(df)
        relabel!(tbl, Dict{Symbol,String}(:x => "VarX", :y => "VarY"))
        @test html_str(tbl) == ref
    end
end

# -----------------------------------------------------------------------
@testset "relabel! Multiline" begin
    df = DataFrame(;
    ...
    run_reftest(tbl, "references/relabel/multiline_label")
...
# -----------------------------------------------------------------------
@testset "relabel! function form" begin
    ...
    run_reftest(tbl, "references/relabel/fn_uppercase_all")
    ...
    run_reftest(tbl, "references/relabel/fn_titlecase_all")

    # Reference test 3: Symbol selector — relabel only a subset
    tbl = StyledTable(df)
    relabel!(tbl, [:first_name]) do col
        titlecase(replace(col, "_" => " "))
    end
    run_reftest(tbl, "references/relabel/fn_symbol_selector")

    # Equality test: String selector produces same output as Symbol selector
    ref = let t = StyledTable(df)
        relabel!(t, [:first_name]) do col
            titlecase(replace(col, "_" => " "))
        end
        html_str(t)
    end
    ...
    @test_throws ArgumentError relabel!(uppercase, StyledTable(df), [:typo])

    # Unit test: empty selector is a silent no-op (no entries written to col_labels)
    tbl = StyledTable(df)
    relabel!(uppercase, tbl, Symbol[])
    ...
    let df = DataFrame(; first_name = ["Alice"], last_name = ["Smith"])
        tbl = StyledTable(df)
        relabel!(uppercase, tbl, :first_name)
...
@testset "align!" begin
    ...
    tbl = StyledTable(df)
    align!(tbl, :x => :center, :y => :center)
    run_reftest(tbl, "references/align/center_both")

    # --- apply-to-all form ---
    tbl = StyledTable(df)
    align!(tbl, :right)
    run_reftest(tbl, "references/align/right_all")

    # --- pair form: single col ---
    tbl = StyledTable(df)
    align!(tbl, :x => :left)
    run_reftest(tbl, "references/align/left_one_col")

    # --- dict / vector-of-pairs form ---
    tbl = StyledTable(df)
    align!(tbl, Dict(:x => :center, :y => :center))
    run_reftest(tbl, "references/align/center_both")
    ...
    # --- error cases ---
    @test_throws ArgumentError align!(StyledTable(df), :centre)
    @test_throws ArgumentError align!(StyledTable(df), :x => :centre)
    @test_throws ArgumentError align!(StyledTable(df), :nonexistent => :left)

    @testset "mixed-type pairs error" begin
        let df = DataFrame(; x = [1], y = [2])
            # Mixed value types: Symbol and String alignment values
            @test_throws ArgumentError align!(
                StyledTable(df),
...
@testset "hide!" begin
    ...
    tbl = StyledTable(df)
    hide!(tbl, :c)
    run_reftest(tbl, "references/hide/single")

    tbl = StyledTable(df)
    hide!(tbl, :a, :c)
    run_reftest(tbl, "references/hide/multiple")

    @test_throws ArgumentError hide!(StyledTable(df), :nonexistent)
end
