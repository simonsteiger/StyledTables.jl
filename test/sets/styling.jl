@testset "style! reference tests" begin
    df = DataFrame(;
        label = ["A", "B", "C"],
    ...
    run_reftest(tbl, "references/style/conditional_color")
    ...
    run_reftest(tbl, "references/style/conditional_bold")

    # Composition: static bold baseline + conditional color
    tbl = StyledTable(df)
    style!(tbl, :change; bold = true) do val
        if val > 0
            (; color = :green)
        elseif val < 0
            (; color = :red)
    ...
    run_reftest(tbl, "references/style/composition_bold_and_color")
    ...
    run_reftest(tbl, "references/style/clear_baseline_bold")

    # Function explicitly clears baseline color on negative cells
    tbl = StyledTable(df)
    style!(tbl, :change; color = :green) do val
        val < 0 ? (; color = nothing) : nothing
    end
    run_reftest(tbl, "references/style/clear_baseline_color")

    # Gradient via Colors.jl range
    GRADIENT = range(colorant"white"; stop = colorant"#1a7340", length = 100)
    tbl = StyledTable(df)
    style!(tbl, :score) do val
        idx = round(Int, clamp(val, 0.0, 1.0) * 99) + 1
        (; color = GRADIENT[idx])
    end
    run_reftest(tbl, "references/style/gradient")

    # Static Symbol color input
    tbl = StyledTable(df)
    style!(tbl, :change; color = :blue)
    run_reftest(tbl, "references/style/static_symbol_color")

    # Static AbstractString color input (CSS name)
    tbl = StyledTable(df)
    style!(tbl, :change; color = "green")
    run_reftest(tbl, "references/style/static_string_color")

    # Static Colorant input
    tbl = StyledTable(df)
    style!(tbl, :change; color = colorant"#1a7340")
    run_reftest(tbl, "references/style/static_colorant")

    # Function returns nothing on all cells; static baseline still applies
    tbl = StyledTable(df)
    style!(tbl, :change; color = :blue) do val
        nothing
    end
    run_reftest(tbl, "references/style/fn_nothing_with_baseline")

    # Repeated calls: second function-only call overwrites function, static baseline survives
    tbl = StyledTable(df)
    f1 = val -> (; color = :red)
    f2 = val -> (; color = :green)
    style!(f1, tbl, :change; bold = true)
    style!(f2, tbl, :change)
    @test tbl.col_style_fns[:change] === f2
    @test tbl.col_styles[:change].bold == true
    run_reftest(tbl, "references/style/repeated_fn_overwrites")

    # Repeated calls: second static-only call updates style, previous function survives
    tbl = StyledTable(df)
    f = val -> (; color = :green)
    style!(f, tbl, :change; bold = true)
    style!(tbl, :change; bold = false)
    @test tbl.col_style_fns[:change] === f
    @test tbl.col_styles[:change].bold == false
    run_reftest(tbl, "references/style/repeated_static_overwrites")
end
...
@testset "style!" begin
    ...
    tbl = StyledTable(df)
    style!(tbl, [:value]; bold = true)
    run_reftest(tbl, "references/style/bold_col")

    tbl = StyledTable(df)
    style!(tbl, [:label]; color = "#FF0000", italic = true)
    run_reftest(tbl, "references/style/color_italic")

    @test_throws ArgumentError style!(StyledTable(df), [:nonexistent]; bold = true)
    ...
    let df = DataFrame(; label = ["A", "B"], value = [1.0, 2.0])
        ...
        tbl = StyledTable(df)
        style!(tbl, :label, :value; bold = true)
        ref2 = let t = StyledTable(df)
            style!(t, [:label, :value]; bold = true)
            html_str(t)
        end
...
@testset "style! static — widened color type" begin
    df = DataFrame(; x = [1, 2, 3])

    tbl = StyledTable(df)
    style!(tbl, :x; color = :red)
    @test tbl.col_styles[:x].color isa String
    @test startswith(tbl.col_styles[:x].color, "#")

    tbl2 = StyledTable(df)
    style!(tbl2, :x; color = colorant"#1a7340")
    @test tbl2.col_styles[:x].color isa String
    @test startswith(tbl2.col_styles[:x].color, "#")

    @test_throws ArgumentError style!(StyledTable(df), :x; color = 42)
end

@testset "style! function form — call time" begin
    df = DataFrame(; change = [-0.1, 0.0, 0.3])
    f = val -> val > 0 ? (; color = :green) : nothing

    # Column validation
    @test_throws ArgumentError style!(x -> nothing, StyledTable(df), [:nonexistent])
    ...
    tbl3 = StyledTable(df)
    style!(tbl3, :change; bold = true)
    style!(x -> nothing, tbl3, :change)
