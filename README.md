# StyledTables

<div align="center">
    <picture>
      <img alt="StyledTables.jl logo" 
        src="/docs/src/assets/logo.svt" width="150">
    </picture>
</div>

[![Build Status](https://github.com/simonsteiger/StyledTables.jl/actions/workflows/CI.yml/badge.svg?branch=main)](https://github.com/simonsteiger/StyledTables.jl/actions/workflows/CI.yml?query=branch%3Amain)

StyledTables.jl is a Julia package building on top of SummaryTables.jl
Use StyledTables.jl to apply custom formatting to tables and render them 
in HTML, docx, LaTeX and Typst formats.
