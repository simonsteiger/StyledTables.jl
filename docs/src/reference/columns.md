...
df |> StyledTable |>
    cols_label(bmi = "BMI (kg/m²)", sbp = "Systolic BP (mmHg)") |> render
```
...
```@example columns
df |> StyledTable |>
    cols_align(:right, [:bmi, :sbp]) |> render
```
...
df |> StyledTable |>
    ...
    fmt_percent(:pct_score; digits = 0) |> render
```
...
# Move :name to front
df |> StyledTable |>
    cols_move([:name]) |> render
```

```@example columns
# Move :value after :name
df |> StyledTable |>
    cols_move([:value]; after = :name) |> render
```
